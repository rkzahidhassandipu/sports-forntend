...
export default function ActiveSessionTracker({ memberId, onComplete }: Props) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 text-[#f0f7ec] shadow-2xl flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-2xl font-black italic tracking-tight">Active <span className="text-[#9AD872]">Tracker</span></h2>
            <p className="text-[#7a9c6e] text-[10px] font-black uppercase tracking-[0.2em]">Member: {memberId}</p>
          </div>
        </div>

        <div className="flex items-center gap-10">
          <div className="text-center">
            <p className="text-[10px] font-black text-[#7a9c6e] uppercase tracking-widest mb-1">Time</p>
            <p className="text-3xl font-black font-mono">{formatTime(seconds)}</p>
          </div>
          <div className="text-center">
            <p className="text-[10px] font-black text-[#7a9c6e] uppercase tracking-widest mb-1">Vol (kg)</p>
            <p className="text-3xl font-black text-[#9AD872]">{totalVolume.toLocaleString()}</p>
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 gap-6">
        {workout.map((exercise) => (
          <div key={exercise.id} className="bg-[#162513] border border-slate-200/60 rounded-[2.5rem] p-8">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-900 tracking-tight">{exercise.name}</h3>
              <span className="text-[10px] font-black text-[#7a9c6e] uppercase tracking-widest">Strength Phase</span>
            </div>

            <div className="space-y-3">
              <div className="grid grid-cols-4 px-4 text-[10px] font-black text-[#7a9c6e] uppercase tracking-widest">
                <span>Set</span>
                <span>Target</span>
                <span>Weight</span>
              ...
              {exercise.sets.map((set, idx) => (
                ...
                >
                  ...
                  <span className="text-xs font-bold text-[#7a9c6e] italic">80% 1RM</span>
                  <span className="font-black text-slate-900">{set.weight}kg x {set.reps}</span>
                  <div className="flex justify-end">
                    <div className={cn(
                      "w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all",
                      set.completed ? "bg-slate-900 border-slate-900 text-[#9AD872]" : "border-slate-300"
                    )}>
                      {set.completed && "✓"}
                    </div>
                  </div>
      ...
      <div className="flex justify-end items-center gap-6 pt-6 border-t border-slate-200">
        <button 
          onClick={onComplete}
          className="text-[10px] font-black uppercase tracking-widest text-[#7a9c6e] hover:text-rose-500 transition-all"
        >
          Discard Session
        </button>
        <button 
          ...
          className="px-10 py-5 bg-slate-900 text-[#f0f7ec] rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-xl disabled:opacity-50"
        >
