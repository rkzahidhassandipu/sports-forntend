    ...
    <div className="relative space-y-8 p-6 lg:p-10 bg-slate-50 min-h-screen">
      ...
      {isSessionModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
          ...
          <div className="relative w-full max-w-5xl max-h-[90vh] overflow-y-auto bg-[#f8fafc] rounded-[3rem] shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-10 duration-500">
            ...
            <button 
              onClick={() => setIsSessionModalOpen(false)}
              className="absolute top-8 right-8 z-[110] w-12 h-12 bg-white rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500 transition-all"
            >
              ✕
            </button>
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button 
          ...
          className="bg-slate-900 text-white px-8 py-4 rounded-[1.5rem] text-xs font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200"
        >
          + New Fitness Entry
        </button>
      </header>
      ...
      <section className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
        <div className="max-w-md mb-10">
          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Search Member by ID</label>
          <input 
            type="text" 
            placeholder="e.g. MEM-10293"
            value={selectedMemberId}
            onChange={(e) => setSelectedMemberId(e.target.value)}
            className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 font-bold outline-none focus:ring-4 focus:ring-lime-500/10 focus:border-lime-500 transition-all"
          />
        </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            {(userRole === "COACH" || userRole === "ADMIN") && (
              <div className="bg-slate-900 text-white p-10 rounded-[2.5rem] relative overflow-hidden">
                ...
                <h4 className="text-lime-400 font-black text-[10px] uppercase tracking-[0.3em] mb-8">Advanced Analytics Overview</h4>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    {Object.entries(report?.metrics || {}).map(([key, val]: any) => (
                      <div key={key} className="relative group">
                        <div className="absolute -left-4 top-0 bottom-0 w-1 bg-lime-500/30 group-hover:bg-lime-500 transition-colors" />
