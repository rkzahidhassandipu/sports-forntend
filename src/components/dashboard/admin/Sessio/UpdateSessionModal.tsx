...
export default function UpdateSessionModal({ session, isOpen, onClose }: UpdateProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-8 shadow-2xl animate-in fade-in zoom-in duration-200 overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Start</label>
              <input type="time" {...register("startTime")} className="w-full p-3 border border-slate-100 rounded-xl" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">End</label>
              <input type="time" {...register("endTime")} className="w-full p-3 border border-slate-100 rounded-xl" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Cap</label>
          ...
          <div className="pt-4">
            <button 
              ...
              className="w-full py-4 bg-slate-900 text-white font-black rounded-2xl hover:bg-lime-500 hover:text-slate-950 transition-all shadow-lg shadow-slate-200 disabled:opacity-50"
            >
