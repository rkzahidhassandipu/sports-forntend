    ...
    <div className="min-h-screen pt-20 pb-16">
      ...
      <div className="bg-brand-mid border-b border-brand-border py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          {/* Search */}
          <div className="max-w-xl mx-auto flex items-center gap-3 bg-brand-card border border-brand-border rounded-2xl px-5 py-3.5 focus-within:border-lime-500 transition-all">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="mb-14">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {HELP_CATEGORIES.map((cat) => (
                <div
                  key={cat.title}
                  className="bg-brand-mid border border-brand-border rounded-xl p-5 hover:border-lime-500/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-lime-500/5 transition-all duration-200 cursor-pointer"
                >
                  ...
                  <ul className="flex flex-col gap-2">
                    {cat.articles.map((article) => (
                      <li key={article}>
                        <button className="text-xs text-muted-foreground hover:text-lime-500 transition-colors text-left w-full leading-relaxed">
                          → {article}
                        </button>
                      </li>
                    ))}
              ...
              {filtered.map((faq, i) => (
                <div key={i} className="bg-brand-mid border border-brand-border rounded-xl overflow-hidden">
                  <button
                    ...
                    className="w-full px-5 py-4 flex items-center justify-between text-left gap-4 hover:text-lime-500 transition-colors duration-200"
                  >
                    <span className="text-sm font-medium">{faq.q}</span>
                    <span
                      className={cn(
                        "text-xl text-muted-foreground transition-transform duration-300 flex-shrink-0 leading-none",
                        openFaq === i && "rotate-45 text-lime-500"
                      )}
                    >
                      +
                    </span>
        ...
        {/* Contact support CTA */}
        <div className="mt-14 bg-gradient-to-br from-lime-500/8 to-teal-500/5 border border-lime-500/15 rounded-2xl p-8 sm:p-12 text-center max-w-2xl mx-auto">
          <div className="text-4xl mb-4">💬</div>
          <h3 className="font-display font-black text-3xl mb-3">
            STILL NEED <span className="text-lime-500">HELP?</span>
          </h3>
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            <Link
              href="/contact"
              className="bg-lime-500 text-brand-dark font-display font-bold px-6 py-3 rounded-xl hover:bg-lime-400 transition-all duration-200 active:scale-95 tracking-wide text-sm"
            >
              CONTACT SUPPORT
            </Link>
            <a
              href="mailto:support@sportpulse.com.bd"
              className="border border-brand-border text-muted-foreground font-medium px-6 py-3 rounded-xl hover:border-lime-500/50 hover:text-lime-500 transition-all duration-200 text-sm"
            >
              ✉ Email Us
            </a>
          </div>
