...
function Pagination({
  ...
  return (
    <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4">
      {/* Count info */}
      <p className="text-[11px] font-bold text-[#7a9c6e] uppercase tracking-widest">
        Showing{" "}
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          className="h-9 w-9 flex items-center justify-center rounded-xl border border-slate-200 text-[#7a9c6e] hover:bg-slate-900 hover:text-[#0f1a0d] hover:border-slate-900 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
          ...
          item === "ellipsis" ? (
            ...
            <button
              ...
              className={cn(
                ...
                item === currentPage
                  ? "bg-slate-900 text-[#f0f7ec] border border-slate-900 shadow-md"
                  : "border border-slate-200 text-[#7a9c6e] hover:bg-slate-900 hover:text-[#0f1a0d] hover:border-slate-900",
              )}
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 p-6 lg:p-10">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div className="space-y-1">
            ...
            <h1 className="text-4xl font-black tracking-tight text-slate-900">
              Editorial{" "}
              <span className="font-serif italic font-light text-[#7a9c6e]">
                Archives
              </span>
            </h1>
          </div>
          ...
          <button
            ...
            className="group flex items-center gap-2 h-14 px-8 bg-[#162513] text-[#f0f7ec] rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-[#9AD872] transition-all duration-300 shadow-xl "
          >
            <Plus
              size={18}
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-10">
          {/* Tab switcher */}
          <div className="flex bg-[#162513] p-1.5 rounded-2xl border border-slate-200 shadow-sm">
            {(["ALL", "PUBLISHED", "DRAFT"] as TabKey[]).map((tab) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab
                    ? "bg-slate-900 text-[#f0f7ec] shadow-md"
                    : "text-[#7a9c6e] hover:text-[#7a9c6e] hover:bg-slate-50",
                )}
              >
                {tab}
              </button>
          ...
          <div className="relative flex-grow group">
            ...
            <input
              ...
              className="w-full h-full py-4 pl-12 pr-4 bg-[#162513] border border-slate-200 rounded-2xl text-sm focus:border-[#9AD872] outline-none transition-all shadow-sm"
        ...
        <div className="space-y-4">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-24 text-[#7a9c6e] space-y-4">
            ...
            <div className="py-24 bg-[#162513] rounded-[2.5rem] border border-slate-100 flex flex-col items-center justify-center text-slate-300">
            ...
            posts.map((post: BlogPost) => (
              <article
                key={post.id}
                className="group bg-[#162513] border border-slate-200 rounded-3xl p-5 hover:border-slate-400 transition-all flex flex-col md:flex-row items-center gap-6 shadow-sm"
              >
                {/* Cover image or icon */}
                ...
                <div className="flex-grow space-y-2 text-center md:text-left min-w-0">
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                    ...
                    {post.tags?.slice(0, 3).map((tag) => (
                      <span
                        key={tag}
                        className="text-[9px] bg-slate-50 text-[#7a9c6e] px-2 py-1 rounded-lg font-bold uppercase tracking-tighter"
                      >
                        #{tag}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 text-[11px] font-bold text-[#7a9c6e] uppercase tracking-tight">
                    <div className="flex items-center gap-1.5">
                      <Eye size={14} />
                ...
                <div className="flex gap-2 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 flex-shrink-0">
                  <Link href={`/dashboard/admin/blogs/${post.id}`}>
                    <span className="h-11 w-11 rounded-xl bg-slate-50 text-[#7a9c6e] flex items-center justify-center hover:bg-slate-900 hover:text-[#0f1a0d] transition-all cursor-pointer">
                      <Edit3 size={18} />
                    </span>
                  </Link>
                  ...
                  <button
                    ...
                    className="h-11 w-11 rounded-xl bg-rose-50 text-rose-400 flex items-center justify-center hover:bg-rose-500 hover:text-[#0f1a0d] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                  >
