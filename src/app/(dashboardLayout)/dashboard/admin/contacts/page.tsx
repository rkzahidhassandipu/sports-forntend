      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1 font-medium italic">Manage client messages, partnership leads, and support requests.</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden md:block text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">New Leads Today</p>
              <p className="text-xl font-black text-lime-600">+12</p>
            </div>
            <div className="w-12 h-12 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-xl shadow-sm">✉️</div>
          </div>
        </header>

        {/* Action Bar */}
        <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-3 rounded-2xl border border-slate-200/60 shadow-sm">
            ...
            {["ALL", "UNREAD", "ARCHIVED"].map((filter) => (
              <button
                ...
                className={cn(
                  ...
                  activeFilter === filter ? "bg-white text-slate-950 shadow-sm" : "text-slate-400 hover:text-slate-600"
                )}
              >
                {filter}
              </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Message Preview</th>
                ...
                <tr key={i} className={cn("hover:bg-slate-50/30 transition-colors group cursor-pointer", contact.unread && "bg-lime-50/20")}>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xs", contact.unread ? "bg-lime-500 text-slate-950" : "bg-slate-100 text-slate-400")}>
                  ...
                  <td className="px-8 py-6">
                    <div className="max-w-xs lg:max-w-md">
                      ...
                      <p className="text-xs text-slate-500 line-clamp-1">{contact.msg}</p>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-slate-400 font-medium italic">{contact.date}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {/* markRead route support */}
                      <button className="p-2.5 bg-slate-100 hover:bg-slate-900 hover:text-white rounded-xl text-xs transition-all" title="Mark as Read">👁️</button>
                      {/* deleteContact route support (isAdmin only) */}
                      <button className="p-2.5 bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl text-xs transition-all" title="Delete Inquiry">🗑️</button>
                    </div>
                  </td>
                </tr>
              ))}
